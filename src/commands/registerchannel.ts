...
export const registerchannel: Command = {
  ...
  async execute(interaction: CommandInteraction) {
    try {
      ...
      const timezone = "UTC"; // interaction.options.get('timezone')?.value as string || 'UTC';
      ...
      const hour = 9; // interaction.options.get('hour')?.value as number || 9;
      const minute = 0; // interaction.options.get('minute')?.value as number ||
      ...
      await interaction.editReply({
        content: `✅ Successfully registered <#${channel.id}> for daily quizzes!\n\n` +
